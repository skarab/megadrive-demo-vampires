...
void EFFECT_Bar2(int end)
{
	...
	void vblank()
    	...
    	u16 w = bar2.w;
    	u16 h = bar2.h;
    	VDP_setPalette(PAL0, bar2.palette->data);
    	VDP_loadBMPTileData((u32*)bar2.image, 1, w / 8, h / 8, w/8 );
    	int xs, ys;
	...
	u32 start_time = smeSOUND_GetTime(music);
		...
		for (k=0 ; k<16 ; ++k)
        	{
                	int r = ((u16*)bar2.palette->data)[k]&15;
               	 	int g = (((u16*)bar2.palette->data)[k]>>4)&15;
                	int b = (((u16*)bar2.palette->data)[k]>>8)&15;
