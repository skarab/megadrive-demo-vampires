...
void EFFECT_Kstorm(int end)
{
	int k;
	for (k=0 ; k<16 ; ++k)
        	VDP_setPaletteColor(k, 0);
    		
	//VDP_setPaletteColors(0, kstorm.palette->data, 16);

	u16 w = title.w;
    	u16 h = title.h;
    	VDP_loadBMPTileData((u32*)kstorm.image, 1, w/8, h/8, w/8);
	int ys;
	...
	int start_time = smeSOUND_GetTime(music);
		...
		else if (time>8000-200) fade = 255-(time-(8000-200))*255/200;
		...
		for (k=0 ; k<16 ; ++k)
            	{
                	int r = ((u16*)kstorm.palette->data)[k]&15;
                	int g = (((u16*)kstorm.palette->data)[k]>>4)&15;
                	int b = (((u16*)kstorm.palette->data)[k]>>8)&15;
