...
void EFFECT_Boobs(int end)
{
	...
	VDP_setPaletteColors(0, boobs.palette->data, 16);

	u16 w = title.w;
    	u16 h = title.h;
    	VDP_loadBMPTileData((u32*)boobs.image, 1, w/8, h/8, w/8);
	int ys;
	...
	int ii = 0;
		...
		for (k=0 ; k<16 ; ++k)
            	{
                	int r = ((u16*)boobs.palette->data)[k]&15;
                	int g = (((u16*)boobs.palette->data)[k]>>4)&15;
                	int b = (((u16*)boobs.palette->data)[k]>>8)&15;
